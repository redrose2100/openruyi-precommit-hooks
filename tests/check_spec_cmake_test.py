...
from openruyi_precommit_hooks.check_spec_cmake import main
...
def test_ok_dep_declared(tmp_path):
    f = _write(
        tmp_path,
        'good1.spec',
        'BuildSystem:    cmake\n'
...
def test_ok_dep_with_version(tmp_path):
    f = _write(
        tmp_path,
        'good2.spec',
        'BuildSystem:    cmake\n'
...
def test_ok_not_cmake_skipped(tmp_path):
    # the rule only applies to BuildSystem: cmake
    for bs in ('autotools', 'meson', 'golang', 'pyproject', 'rustcrates'):
        f = _write(
            tmp_path,
            f'good_{bs}.spec',
            f'BuildSystem:    {bs}\n'
...
def test_ok_no_buildrequires(tmp_path):
    # a non-cmake spec without BuildRequires is fine
    f = _write(
        tmp_path,
        'good_nobr.spec',
        'BuildSystem:    golang\n'
...
def test_fail_missing_cmake(tmp_path):
    f = _write(
        tmp_path,
        'bad1.spec',
        'BuildSystem:    cmake\n'
...
def test_fail_error_message_lists_missing(tmp_path):
    ...
    res = subprocess.run(
        [sys.executable, '-m', 'openruyi_precommit_hooks.check_spec_cmake', f],
        capture_output=True,
        text=True,
        check=False,
    )
    assert res.returncode == 1
    assert 'declare cmake' in res.stdout
